...
static const arg_list_item_t remove_service_option_list[] = {
...
int tool_cmd_remove_service(int argc, char* argv[])
{
	...
	while (1) {
		...
		int option_index = 0;
		c = getopt_long(argc, argv, "ht:ds", long_options,
				&option_index);
		...
		switch (c) {
		case 'h':
			print_arg_list_help(remove_service_option_list,
					    argv[0], remove_service_syntax);
			ret = ERRORCODE_HELP;
			goto bail;
